...
namespace Bert.RateLimiters
{
    ...
    public class RollingWindowThrottler
    {
        ...
        public RollingWindowThrottler(int occurrences, TimeSpan timeUnit)
        {
            if (occurrences <= 0)
                throw new ArgumentOutOfRangeException(nameof(occurrences),
                    "Number of occurences must be a positive integer");
        ...
        /// </summary>
        public TimeSpan TimeUnit => TimeSpan.FromTicks(timeUnitTicks);
        ...
        {
            if (tokens <= 0)
                throw new ArgumentOutOfRangeException(nameof(tokens), "Should be positive integer greater than 0");
